...
module.exports = function(grunt) {
    ...
    var cp = require('child_process'),
        ...
        copyOptions = {
            process: function(file, srcPath, destPath) {
                var matches = (/^.*\.(scss|less|css|js|map|png|jpg|gif|eot|svg|ttf|woff|woff2)$/).test(srcPath);
                if (!matches) {
                    return false;
                }
                return file;
            },
            noProcess: ['**/*.{png,gif,jpg,ico,psd,eot,ttf,woff,woff2}']
        };
    ...
    grunt.registerTask('copy-frontend', 'Copy Front-End resources', function() {
        ...
        deps.forEach(function(dep) {
            ...
            if (dep == '@mware/bootstrap') {
                dest = path.join(outputDir, 'bootstrap');
            }
