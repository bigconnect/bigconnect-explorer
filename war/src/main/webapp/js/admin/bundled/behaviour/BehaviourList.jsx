...
], function (bcApi, React, ReactDOM, PropTypes, createReactClass, ReactTableDef, ConfirmDialog) {
    'use strict';

    const ReactTable = ReactTableDef.default;

    const BehaviourList = createReactClass({
        tableInstance: null,
        ...
        handleDelete(event, row) {
            var self = this;
            ConfirmDialog.showDialog('Confirm', 'Are you sure ? This operation is irreversible.')
                .then(() => {
                    bcApi.connect().then(({dataRequest}) => {
                        dataRequest('behaviour', 'delete', row.value)
                            .then(function () {
                    ...
                    <div className="text-center">
                        <div className="btn-group btn-group-sm">
                            ...
                            <button title="Delete" data-placement='bottom' className="btn btn-link showToolTip"
                                ...
                                <i className="material-icons md-18">delete_forever</i></button>
                        </div>
                    </div>
                )
            }];
            ...
            return (
                <div className="panel">
                    <div className="panel-heading">
                        <div className="panel-heading-title">Behaviours</div>
                        <div className="panel-heading-subtitle text-muted">
                            Behaviours are collection of queries with a score
                        </div>
