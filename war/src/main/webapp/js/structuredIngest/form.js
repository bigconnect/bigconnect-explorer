...
define([
    ...
    function StructuredMappingForm() {
        ...
        this.defaultAttrs({
            ...
            noHeaderSelector: '.entities .help button.noheader',
            autoMapSelector: '.entities .help button.automap'
        })
        ...
        this.after('initialize', function() {
            ...
            this.on('click', {
                ...
                autoMapSelector: this.onAutoMapClick
            });
            this.on('mouseover', {
        ...
        this.loadInfo = function(sheetIndex, headerIndex) {
            ...
            return util.analyze(this.attr.vertex ? this.attr.vertex.id : null, options)
        ...
        this.onCreatedObjectsClick = function(event) {
            ...
            if (this.$node.find('.segmented-control .entities.active').length) {
                ...
                Promise.all([
                    ...
                    Promise.require('structuredIngest/createdObjectPopover')
        ...
        this.updatePlaceholder = function() {
            ...
            if (this.$node.hasClass('mode-header')) {
                ...
                $placeholder.text(i18n('csv.file_import.choose-header.placeholder'))
                    .append(
                        $('<button>')
                            .addClass('btn btn-link noheader')
            ...
            if (data) {
                ...
                this.mappedObjects[type] = list = list.map(function(o) {
                    if (o.id === data.object.id) {
                        ...
                        o.automap = data.automap;
                        return data.object;
                    }
                    return o;
                });
                if (shouldCreateNew) {
                    data.object.automap = data.automap;
        ...
        this.onAutoMapClick = function(event) {
            var self = this;
            var $editor = this.willTransitionToEditor('entities', AutoMapper);
            AutoMapper.attachTo($editor, {
                headers: self.headers,
                types: self.headerTypes,
                allHeaders: self.headers,
                vertices: this.mappedObjects.vertices,
                defaultVisibilitySource: this.attr.visibilitySource
            });
            this.performTransitionToEditor($editor);
        };
        ...
        this.performTransitionToEditor = function($editor) {
            ...
            var animationSequence = _.compact([
                ...
                {
                    e: $container,
                    p: { height: parseInt($editor.outerHeight(true), 10) + 'px' },
                    o: {
                        sequenceQueue: false,
        ...
        this.ingestPreview = function() {
            ...
            this.dataRequest('structuredIngest', 'ingest', mapping, self.attr.vertex ? self.attr.vertex.id : null, self.attr.tmpFile, this.parseOptions, this.currentImportActionIsPreview, this.shouldPublish)
                .then(function(result) {
                    ...
                    if (result.success) {
                        ...
                        bcData.storePromise.then(store => {
                            store.dispatch(ingestActions.setLoadType(''));
                        });
