...
define([
    ...
    './SavedSearchTable',
    '../columnConfigPopover',
    'search/cypher/dashboard/searchresults'
], function(
    createReactClass,
    PropTypes,
    SavedSearchTable,
    ...
    const DEFAULT_TAB_SETTINGS = {
        ...
        sortPropertyIri: ONTOLOGY_CONSTANTS.PROP_TITLE,
        columns: [],
        active: false
    };
    const AGGREGATION_FIELD = {
        vertex: ONTOLOGY_CONSTANTS.PROP_CONCEPT_TYPE,
        edge: ONTOLOGY_CONSTANTS.EDGE_LABEL
    };
    ...
    const SavedSearchTableCard = createReactClass({
        ...
        render() {
            ...
            return (
                ...
                >
                    {(() => {
                        if (!searchId) {
                            return <a onClick={configureItem}>{i18n('dashboard.item.table.config.search')}</a>
                        } else if (isLoading(searchId, tableView, tableData)) {
                            const message = isEmpty(searchId, tableData)
                                ? i18n('dashboard.item.table.watermark.empty')
                                : i18n('dashboard.item.table.watermark.loading');
                            ...
                            if (isDefaultSearch(tableView)) {
                                ...
                                CypherSearchResults.teardownAll();
                                CypherSearchResults.attachTo($('.result-table'), { searchId: searchId });
                            }
                        }
                    })()}
                </div>
            ...
            function isDefaultSearch(tableView) {
                return !!tableView.url && !(tableView.url.indexOf('cypher') > -1);
            }

            function isLoading(searchId, tableView, tableData) {
                if (isDefaultSearch(tableView) && searchId) {
        ...
        loadTable() {
            ...
            this.dataRequest('search', 'get', searchId)
                .catch((e) => {
                ...
                .then((search) => {
                    ...
                    if (tableView.url.indexOf('cypher') > -1) {
                        return new Promise(function(resolve, reject) {
                            resolve("done");
                        });
                    } else {
                        return new Promise(function (resolve, reject) {
                            resolve(this.loadTabs());
                        }.bind(this))
                            .then((tabs) => {
        ...
        loadRows(startIndex, stopIndex) {
            ...
            function transformResultsToRows(results) {
                ...
                const F = self.props.bcApi.v1.formatters;
                    ...
                    return Promise.map(columns, ({ title }) => {
                        ...
                        function getColumnValues(properties) {
                            return Promise.map(properties, (p) => {
                                ...
                                if (p.name === ONTOLOGY_CONSTANTS.PROP_TITLE) {
    ...
    function transformItemToTable(iri, type, ontology) {
        ...
        return _.chain(item.properties)
            ...
            .sortBy((p) => {
                if (p.title === ONTOLOGY_CONSTANTS.PROP_TITLE || !p.displayName) {
                    return Number.MAX_VALUE;
                }
                return p.displayName;
            })
            .map(({ title, displayName }) => {
                let width = displayName ? (displayName.length * FONT_SIZE) : COLUMN_WIDTH;
                return {
                    title: title,
