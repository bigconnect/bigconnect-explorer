    ...
    function StackedBar() {
        ...
        this.processData = function(data) {
            ...
            if (root.orderedByNestedAgg) {
                ...
                buckets = _.sortBy(root.buckets, (d) => {
                    if ('count' in d.value) {
            ...
            var bucket = _.first(root.buckets);

            this.isNested = Boolean(bucket && bucket.value.nested && bucket.value.nested.length);
            this.isHorizontal = this.attr.item.configuration.reportRenderer === 'org-bigconnect-bar-horizontal';
