...
public class HandlerChain {
    ...
    public void next(HttpServletRequest request, HttpServletResponse response, Throwable t) throws Exception {
        if (nextHandlerIndex < handlers.length) {
            RequestResponseHandler handler = handlers[nextHandlerIndex];
            nextHandlerIndex++;

            if(handler instanceof RequestResponseExceptionHandler)
                ((RequestResponseExceptionHandler)handler).handle(request, response, this, t);
            else
    ...
    public void next(HttpServletRequest request, HttpServletResponse response) throws Exception {
        this.next(request, response, null);
    }
}
