...
public class ListWatches implements ParameterizedHandler {
    ...
    @Inject
    public ListWatches(final WatchlistRepository watchlistRepository) {
        this.watchlistRepository = watchlistRepository;
    }

    @Handle
    public JSONObject handle(User user) throws Exception {
        Stream<Watch> userWatches = watchlistRepository.getUserWatches(user);

        JSONArray entries = new JSONArray();
        JSONObject results = new JSONObject();
        userWatches.forEach(watch ->
                entries.put(watch.toJson()));
