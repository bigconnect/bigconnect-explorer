...
public class DataSourcePreview implements ParameterizedHandler {
    private static final BcLogger LOGGER = BcLoggerFactory.getLogger(DataSourcePreview.class);
    private DataConnectionRepository dataConnectionRepository;

    @Inject
    public DataSourcePreview(DataConnectionRepository dataConnectionRepository) {
        this.dataConnectionRepository = dataConnectionRepository;
    }

    @Handle
    public ClientApiDataSourcePreview handle(
            User authUser,
            @Required(name = "dcId") String id,
            @Required(name = "sqlSelect") String sqlSelect
    ) throws Exception {
        ...
        try {
            ...
            if (dc == null) {
                throw new BcException("Cannot find DataConnection with id=" + id);
            }

            preview = dataConnectionRepository.getDataSourceManager().generatePreview(dc, sqlSelect);
        } catch (BcException e) {
            LOGGER.error("BCException ", e);
            preview.setSuccess(false);
            preview.setExceptionMessage(e.getMessage());
        }
        return preview;
