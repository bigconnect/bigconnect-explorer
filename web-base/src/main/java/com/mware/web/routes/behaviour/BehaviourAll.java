...
public class BehaviourAll implements ParameterizedHandler {
    ...
    @Inject
    public BehaviourAll(final BehaviourRepository behaviourRepository) {
        this.behaviourRepository = behaviourRepository;
    }

    @Handle
    public JSONArray handle() throws Exception {
        List<ClientApiBehaviour> clientApiBehaviours = StreamUtil.stream(behaviourRepository.getAllBehaviours())
                .map(b -> behaviourRepository.toClientApi(b))
                .collect(Collectors.toList());

        JSONArray result = new JSONArray();
        clientApiBehaviours.forEach(cbh -> {
            String json = ClientApiConverter.clientApiToString(cbh);
