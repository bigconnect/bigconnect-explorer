...
public class BehaviourGetById implements ParameterizedHandler {
    ...
    @Handle
    public ClientApiBehaviour handle(
            @Required(name = "id") String id
    ) {
        Preconditions.checkNotNull(id, "behaviour id cannot be null");

        Behaviour behaviour = behaviourRepository.findById(id);
        Preconditions.checkNotNull(behaviour, "No behaviour with id="+id+" was found");

        return behaviourRepository.toClientApi(behaviour);
    }
}
