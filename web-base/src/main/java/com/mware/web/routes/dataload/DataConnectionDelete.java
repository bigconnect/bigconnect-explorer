...
public class DataConnectionDelete implements ParameterizedHandler {
    ...
    public ClientApiSuccess handle(
            User authUser,
            @Optional(name = "dcId") String dcId,
            @Optional(name = "dsId") String dsId
    ) throws Exception {
        if(!StringUtils.isEmpty(dcId)) {
            dataConnectionRepository.deleteDataConnection(dcId);
        } else if(!StringUtils.isEmpty(dsId)) {
            dataConnectionRepository.deleteDataSource(dsId);
        }
