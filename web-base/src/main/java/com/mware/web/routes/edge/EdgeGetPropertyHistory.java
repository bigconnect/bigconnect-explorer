...
public class EdgeGetPropertyHistory implements ParameterizedHandler {
    ...
    public ClientApiHistoricalPropertyResults handle(
        ...
        if (edge == null) {
            throw new BcResourceNotFoundException(String.format("edge %s not found", graphEdgeId));
        }

        Property property = edge.getProperty(propertyKey, propertyName);
        if (property == null) {
            throw new BcResourceNotFoundException(String.format("property %s:%s not found on edge %s", propertyKey, propertyName, edge.getId()));
        }
