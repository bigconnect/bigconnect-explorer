...
import com.mware.core.ingest.database.DataSource;
...
public class DataSourceSave implements ParameterizedHandler {
    ...
    @Handle
    public ClientApiDataSource handle(
            @Required(name = "data") ClientApiDataSource params
    ) throws Exception {
        DataSource ds = null;
        if(!StringUtils.isEmpty(params.getDsId())) {
            dataConnectionRepository.updateDataSource(params);
            ds = dataConnectionRepository.findDsById(params.getDsId());
        } else {
            ds = dataConnectionRepository.createDataSource(params);
        }

        return dataConnectionRepository.toClientApi(params.getDsId(), ds);
    }
}
