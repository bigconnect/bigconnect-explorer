...
public class DataConnectionGetById implements ParameterizedHandler {
    ...
    @Handle
    public ClientApiDataConnection handle(
            User authUser,
            @Required(name = "dcId") String id
    ) throws Exception {
        DataConnection dc = dataConnectionRepository.findDcById(id);
        if (dc == null) {
            throw new BcResourceNotFoundException("Data Connection with id="+id+" was not found");
        }

        ClientApiDataConnection clientApi = dataConnectionRepository.toClientApi(dc);
        return clientApi;
    }
}
