...
public class RegexTable implements ParameterizedHandler {
    ...
    @Handle
    public ClientApiRegexes handle(
            User user,
            @Required(name = "pageSize") int pageSize,
            @Required(name = "page") int page
    ) throws Exception {
        List<Regex> regexes = toList(regexRepository.getAllRegexes());
        return regexRepository.toClientApi(regexes);
    }
}
