...
public class AllowNoneAclProvider extends ACLProvider {
    ...
    protected void appendACL(ClientApiElement clientApiElement, Set<String> privileges, User user, String workspaceId) {
        for (ClientApiProperty apiProperty : clientApiElement.getProperties()) {
            apiProperty.setUpdateable(false);
            apiProperty.setDeleteable(false);
            apiProperty.setAddable(false);
        }
        clientApiElement.setUpdateable(false);
        clientApiElement.setDeleteable(false);
