...
define([
    ...
    const edgeDisplay = (label, ontologyRelationships, edges) => {
        const display = label in ontologyRelationships ? ontologyRelationships[label].displayName : label;
        const showNum = edges.length > 1;
        const num = showNum ? ` (${F.number.pretty(edges.length)})` : '';
        return display + num;
    };
    const edgeColor = (label, ontologyRelationships) => {
        const color = label in ontologyRelationships ? ontologyRelationships[label].color : '#232323';
        return color;
    };
    ...
    const Graph = createReactClass({
        ...
        propTypes: {
            ...
            nodeLabels: PropTypes.bool,
            nodeImages: PropTypes.bool,
        ...
        componentWillReceiveProps(nextProps) {
            ...
            if (nextProps.concepts !== this.props.concepts || nextProps.properties !== this.props.properties) {
                clearForOntologyUpdates.push(
                    'vertexToCyNode',
                    'org.bigconnect.graph.node.class',
                    'org.bigconnect.graph.node.transformer',
                    'org.bigconnect.graph.node.decoration#applyTo',
                    'org.bigconnect.graph.node.decoration#data'
                )
            }
            if (nextProps.relationships !== this.props.relationships) {
                clearForOntologyUpdates.push(
                    'vertexToCyNode',
                    'org.bigconnect.graph.edge.transformer',
                    'org.bigconnect.graph.edge.class')
            }
        ...
        render() {
            var { draw, paths } = this.state,
                { panelPadding, registry, workspace, product, interacting, hasPreview, needsLayout } = this.props,
                { editable } = workspace,
                ...
                extensionViews = registry['org.bigconnect.graph.view'];

            return (
                <div ref={r => {this.node = r}} className="org-bigconnect-graph" style={{ height: '100%' }}>
                    <Cytoscape
        ...
        onFocusPaths(event, data) {
            ...
            if (data.paths.length > MaxPathsToFocus) {
                ...
                $(document).trigger('displayInformation', {
                    message: i18n('org.bigconnect.web.product.graph.findPath.too.many', MaxPathsToFocus)
                })
            }
        ...
        onDecorationEvent(event) {
            ...
            if (decoration) {
                const handlerName = {
                    /**
                     * @callback org.bigconnect.graph.node.decoration~onClick
                    ...
                    tap: 'onClick',
                    /**
                     * @callback org.bigconnect.graph.node.decoration~onMouseOver
                     * @this The decoration cytoscape node
                     * @param {object} event The {@link http://js.cytoscape.org/#events/event-object|Cytoscape event} object
                     * @param {object} data
                     * @param {object} data.vertex The vertex this decoration
                    ...
                    /**
                     * @callback org.bigconnect.graph.node.decoration~onMouseOut
                     * @this The decoration cytoscape node
                     * @param {object} event The {@link http://js.cytoscape.org/#events/event-object|Cytoscape event} object
                     * @param {object} data
                     * @param {object} data.vertex The vertex this decoration
        ...
        onMenuSelect(select) {
            ...
            switch (select) {
                ...
                case 'edges':
                    ...
                    break;
                case 'orphan':
                    let inVertices = _.map(productElementIds.edges, e => e.inVertexId);
                    let outVertices = _.map(productElementIds.edges, e => e.outVertexId);
                    let verticesWithEdges = _.union(inVertices, outVertices);
                    selection.vertices = _.chain(productElementIds.vertices)
                        .map(v => v.id)
                        .difference(verticesWithEdges)
                        .value();
                    selection.edges = [];
                    break;
                case 'none':
                    ...
                    return;
                default:
                    const selector = registry['org.bigconnect.graph.selection'].find(e => e.identifier === select);
                    if (selector) {
        ...
        onMenuExport(componentPath) {
            var exporter = _.findWhere(
                    this.props.registry['org.bigconnect.graph.export'],
                    { componentPath }
                );
        ...
        onEditCollapsedNode(collapsedNodeId) {
            ...
            if (this.props.workspace.editable && collapsedNode) {
                Promise.require('com/mware/web/product/graph/popovers/collapsedNode/collapsedNodePopoverShim')
        ...
        onLayoutStop() {
            ...
            this.props.onNeedsLayout(this.props.product.id, false);
        },
        ...
        mapPropsToElements(editable) {
            const { selection, ghosts, productElementIds, elements, relationships, registry, focusing, product, concepts } = this.props;
            ...
            const cyNodeConfig = (node) => {
                ...
                if (ancillary) {
                    ...
                    selectable = false;
                    classes = 'ancillary';
                    registry['org.bigconnect.graph.ancillary'].forEach(({
                    ...
                    classes = mapVertexToClasses(id, vertices, focusing, hovering, registry['org.bigconnect.graph.node.class']);
                    data = mapVertexToData(id, vertices, concepts, registry['org.bigconnect.graph.node.transformer']);
                    ...
                    classes = mapCollapsedNodeToClasses(id, collapsedNodes, focusing, vertexIds, hovering, registry['org.bigconnect.graph.collapsed.class']);
            ...
            const cyVertices = filterByRoot(productVertices).reduce((nodes, nodeData) => {
                ...
                if (id in vertices) {
                    ...
                    const applyDecorations = !ancillary && memoizeFor('org.bigconnect.graph.node.decoration#applyTo', vertex, () => {
                        return _.filter(registry['org.bigconnect.graph.node.decoration'], function(e) {
                            /**
                             * @callback org.bigconnect.graph.node.decoration~applyTo
                             * @param {object} vertex
                    ...
                    if (applyDecorations && applyDecorations.length) {
                        ...
                        const decorations = memoizeFor('org.bigconnect.graph.node.decoration#data', vertex, () => {
                            return applyDecorations.map((dec, index) => {
                                const data = mapDecorationToData(dec, vertex, data => {
                                    if (data) {
                                        const cache = memoizeGet('org.bigconnect.graph.node.decoration#data', vertex);
                ...
                if (ghosts) {
                    _.mapObject(ghosts, (ghost, ghostId) => {
                        if (cyNode.data.vertexIds.includes(ghostId)) {
                            const ghostData = {
                                    ...mapVertexToData(ghostId, vertices, null, registry['org.bigconnect.graph.node.transformer']),
                                parent: rootNode.id,
                            ...
                            nodes.push({
                                ...cyNode,
                                data: ghostData,
                                    classes: mapVertexToClasses(ghostId, vertices, focusing, hovering, registry['org.bigconnect.graph.node.class']),
            ...
            const cyEdges = _.chain(productEdges)
                        ...
                        const multiEdgeLabel = (edges) => {
                            ...
                            const display = edges[0] ?
                                edges[0].label in relationships ?
                                    relationships[edges[0].label].displayName : edges[0].label :
                                edges[0].label;

                            return numTypes === 1 ?
                                i18n('org.bigconnect.web.product.graph.multi.edge.label.single.type', edges.length, display) :
                                i18n('org.bigconnect.web.product.graph.multi.edge.label', edges.length, numTypes);
                        }
                        let classes = 'e';
                ...
                .map(data => {
                    if (data.id) {
                        ...
                        return {
                            data: mapEdgeToData(data, edgesForInfos, relationships, registry['org.bigconnect.graph.edge.transformer']),
                            classes: mapEdgeToClasses(data.edgeInfos, edgesForInfos, focusing, registry['org.bigconnect.graph.edge.class']),
        ...
        showConnectionPopover() {
            ...
            if ('ShowPath' === connectionType) {
                $(document).trigger('defocusPaths');
                $(document).trigger('finishedVertexConnection');

                this.setState({ draw: null }, () => {
                    let dfs = cy.elements().aStar({
                        root: '#'+vertexId,
                        goal: '#'+toVertexId,
                        directed: false
                    });
                    if (dfs) {
                        const cyPath = dfs.path;
                        if (cyPath) {
                            const data = {};
                            data.labels = [];
                            data.sourceId = vertexId;
                            data.targetId = toVertexId;
                            data.processId = '';

                            const path = [];
                            cyPath.forEach(el => {
                                if (el.isNode()) {
                                    path.push(el.data('id'));
                                } else if (el.isEdge()) {
                                    data.labels.push(el.data('label'))
                                }
                            });
                            data.paths = [ path ]
                            data.renderedPaths = [ path ]
    ...
    const mapEdgeToData = (data, edges, ontologyRelationships, transformers) => {
        ...
        return memoizeFor('org.bigconnect.graph.edge.transformer', edges, () => {
            ...
            const base = {
                ...
                edgeColor: edgeColor(label, ontologyRelationships),
                edgeInfos,
                edges
            };
            ...
            if (edges.length) {
                return transformers.reduce((data, fn) => {
                     ...
                     *
                     * @callback org.bigconnect.graph.edge.transformer~transformerFn
    ...
    const mapEdgeToClasses = (edgeInfos, edges, focusing, classers) => {
        ...
        if (edges.length) {
             ...
             *
             * @callback org.bigconnect.graph.edge.class~classFn
            ...
            cls = memoizeFor('org.bigconnect.graph.edge.class', edges, function() {
                const cls = [];
    ...
    const mapDecorationToData = (decoration, vertex, update) => {
        const getData = () => {
             ...
             *
             * @callback org.bigconnect.graph.node.decoration~data
             * @param {object} vertex
    ...
    const mapDecorationToClasses = (decoration, vertex) => {
        ...
        if (_.isString(decoration.classes)) {
            ...
            /**
             * @callback org.bigconnect.graph.node.decoration~classes
             * @param {object} vertex
    ...
    const mapVertexToClasses = (id, vertices, focusing, hovering, classers) => {
        ...
        if (vertexLoaded) {
             ...
             * Mutate the classes array to adjust the classes.
             *
             * @callback org.bigconnect.graph.node.class~classFn
             ...
             * }
             */
            cls = memoizeFor('org.bigconnect.graph.node.class', vertex, function() {
                const cls = [];
    ...
    const mapCollapsedNodeToClasses = (collapsedNodeId, collapsedNodes, focusing, vertexIds, hovering, classers) => {
        ...
        if (loaded) {
             ...
             * Mutate the classes array to adjust the classes.
             *
             * @callback org.bigconnect.graph.collapsed.class~classFn
    ...
    const generateCollapsedNodeTitle = (collapsedNode, vertices, productVertices, collapsedNodes) => {
        ...
        if (vertices) {
            ...
            if (vertex && vertex.length > 1) {
                ...
                title = i18n('org.bigconnect.web.product.graph.collapsedNode.entities.singular');
            }
        }

        return title || i18n('org.bigconnect.web.product.graph.collapsedNode.entities', children.length);
    };

    const vertexToCyNode = (vertex, concepts, transformers) => {
        return memoizeFor('vertexToCyNode', vertex, function() {
            ...
            const concept = concepts[conceptType];
            const conceptColor = (concept && concept.color) || '#292929';
            ...
            return transformers.reduce((data, t) => {
                 ...
                 *
                 * @callback org.bigconnect.graph.node.transformer~transformerFn
    ...
    const mapVertexToData = (id, vertices, concepts, transformers) => {
        if (id in vertices) {
            if (vertices[id] === null) {
                return;
            } else {
                const vertex = vertices[id];
                return vertexToCyNode(vertex, concepts, transformers);
            }
        } else {
            return { id }
        }
    ...
    const CONFIGURATION = (props) => {
        const { pixelRatio, edgeLabels, nodeLabels, nodeImages, product, registry } = props;
        const edgesCount = product.extendedData.edges.length;
        const styleExtensions = registry['org.bigconnect.graph.style'];
        ...
        return {
            ...
            style: styleCache.getOrUpdate(styles, pixelRatio, edgesCount, edgeLabels, nodeLabels, nodeImages, styleExtensions)
        }
    };
    ...
    return RegistryInjectorHOC(Graph, [
        'org.bigconnect.graph.edge.class',
        'org.bigconnect.graph.edge.transformer',
        'org.bigconnect.graph.export',
        'org.bigconnect.graph.node.class',
        'org.bigconnect.graph.node.decoration',
        'org.bigconnect.graph.node.transformer',
        'org.bigconnect.graph.ancillary',
        'org.bigconnect.graph.collapsed.class',
        'org.bigconnect.graph.selection',
        'org.bigconnect.graph.style',
        'org.bigconnect.graph.view'
    ]);
});
