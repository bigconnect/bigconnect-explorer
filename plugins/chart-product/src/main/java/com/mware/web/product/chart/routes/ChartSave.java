...
@Singleton
public class ChartSave implements ParameterizedHandler {
    ...
    public ClientApiChart handle(
        ...
        boolean hasWithSameName =
                StreamUtils.stream(chartRepository.getUserSavedCharts(user))
                        .anyMatch(chart -> name.equals(chart.name));

        if(hasWithSameName) {
            throw new BcException("A chart with the same name already exists: "+name);
        } else {
            return chartRepository.saveChart(name, datasetId, chartData, user);
        }
    }
}
