...
require(['configuration/plugins/registry', 'util/vertex/formatters'], function(registry, F) {
    ...
    registry.registerExtension('org.bigconnect.layout.component', {
        ...
        children: [
            {
                componentPath: 'com/mware/web/product/map/dist/geoShapePreview',
                className: 'org-bigconnect-map-geoshape-preview'
            },
    ...
    function isGeoShapeFile(model) {
        if (F.vertex.displayType(model) === 'document') {
            const mimeType = F.vertex.prop(model, ONTOLOGY_CONSTANTS.PROP_MIME_TYPE);
